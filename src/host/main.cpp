...
int main(int, char**)
{
  ...
  GLFWwindow* window =
    glfwCreateWindow(1280, 720, "tiny-drive-prof", NULL, NULL);
  if (window == NULL)
    return 1;
  ...
  auto fontPath = (get_exe_path().parent_path() / "arial.ttf").u8string();
  ImFont* pFont = io.Fonts->AddFontFromFileTTF(
    fontPath.data(), 16.0f, nullptr, io.Fonts->GetGlyphRangesCyrillic());
  ...
  cr_plugin ctx;
  HostData hostData;
  hostData.imguiContext = ImGui::GetCurrentContext();
  hostData.implotContext = ImPlot::GetCurrentContext();
  hostData.memAlloc = [](size_t size, void*) { return malloc(size); };
  hostData.memFree = [](void* ptr, void*) { return free(ptr); };
  ctx.userdata = &hostData;

  // the full path to the live-reloadable application
  auto guest_dll_path = (get_exe_path().parent_path() / TDP_GUEST).string();
  if (!cr_plugin_open(ctx, guest_dll_path.data())) {
    fprintf(stderr, "Failed to initialize cr for %s!\n",
            guest_dll_path.data());
    return 1;
  }
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    bool check_plugin_updates = (updates++ % 32) == 0;
    ImGui::PushFont(pFont);
    cr_plugin_update(ctx, check_plugin_updates);
    ImGui::PopFont();
